...
export const EditAdminSettingsModal = () => {
  ...
  const isModalOpen = isOpen && modalType === "editSettings";
  ...
  const {
    execute: editAdminSettingsExecute,
    isPending: editAdminSettingsPending,
    reset: editAdminSettingsReset,
  } = useServerAction(EditAdminSettings, {
  ...
  const form = useForm<CreateSettingsFormSchemaType>({
    ...
    defaultValues: {
      ...
      credentialId: settingsData?.credentialId || undefined,
      maxFileSize: Number(settingsData?.maxFileSize) / 1024 / 1024 || 0,
      subFolder: settingsData?.subFolder || "",
      type: settingsData?.type || StorageType.LOCAL,
    },
  });
  ...
  }, [isModalOpen, getAllAppsDataExecute, form, settingsData]);
  ...
  async function onSubmit(values: CreateSettingsFormSchemaType) {
    ...
    await editAdminSettingsExecute(newValues);
  }

  function handleCloseModal() {
    form.reset();
    getAllAppsDataReset();
    editAdminSettingsReset();
    setData({
      allAppsData: [],
      allCredentialsData: [],
    });
