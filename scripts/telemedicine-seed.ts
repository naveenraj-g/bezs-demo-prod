...
import { generateRandomColor } from "@/modules/telemedicine/utils";
...
async function seed() {
  ...
  for (const role of staffRoles) {
    ...
    await prismaTeleMedicine.staff.create({
      data: {
        id: faker.string?.uuid(),
        orgId: "cFC407BNdZAr3AX0vkAXHYsZt1FfcjHf",
  ...
  const doctors = [];
  for (let i = 0; i < 10; i++) {
    const doctor = await prismaTeleMedicine.doctor.create({
      data: {
        id: faker.string.uuid(),
        orgId: "cFC407BNdZAr3AX0vkAXHYsZt1FfcjHf",
        userId: faker.string.uuid(),
  ...
  const patients = [];
  for (let i = 0; i < 20; i++) {
    const patient = await prismaTeleMedicine.patient.create({
      data: {
        id: faker.string.uuid(),
        name: faker.name.fullName(),
        userId: faker.string.uuid(),
